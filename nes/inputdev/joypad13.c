...
static void strobe()
{
	...
	if(joykeys[config.joy_keys[3][0]]) data |= (INPUT_A << 8);
	if(joykeys[config.joy_keys[3][1]]) data |= (INPUT_B << 8);
	if(joykeys[config.joy_keys[3][2]]) data |= (INPUT_SELECT << 8);
	if(joykeys[config.joy_keys[3][3]]) data |= (INPUT_START << 8);
	if(joykeys[config.joy_keys[3][4]]) data |= (INPUT_UP << 8);
	if(joykeys[config.joy_keys[3][5]]) data |= (INPUT_DOWN << 8);
	if(joykeys[config.joy_keys[3][6]]) data |= (INPUT_LEFT << 8);
	if(joykeys[config.joy_keys[3][7]]) data |= (INPUT_RIGHT << 8);
	counter = 0;
}
